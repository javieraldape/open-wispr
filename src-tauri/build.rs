...
fn build_apple_intelligence_bridge() {
    ...
    // Framework-in-SDK is necessary but not sufficient: the @Generable macro in
    // apple_intelligence.swift needs the FoundationModelsMacros compiler plugin,
    // which ships with full Xcode toolchains but NOT the bare Command Line Tools
    // (whose SDK still contains the framework). Probe the toolchain's host plugin
    // directory; without the plugin, compiling the real bridge fails with
    // "plugin for module 'FoundationModelsMacros' not found".
    // APPLE_INTELLIGENCE_BRIDGE=real|stub overrides the autodetection.
    ...
    let source_file = match env::var("APPLE_INTELLIGENCE_BRIDGE").as_deref() {
        Ok("real") => {
            println!("cargo:warning=Building with Apple Intelligence support (forced).");
            REAL_SWIFT_FILE
        }
        Ok("stub") => {
            println!("cargo:warning=Building with Apple Intelligence stubs (forced).");
            STUB_SWIFT_FILE
        }
        _ if has_foundation_models && has_macro_plugin => {
        ...
        _ => {
            if has_foundation_models {
                println!(
                    "cargo:warning=FoundationModels SDK found but the FoundationModelsMacros \
                     compiler plugin is missing (Command Line Tools-only toolchain?). \
                     Building with stubs. Set APPLE_INTELLIGENCE_BRIDGE=real to override."
                );
            } else {
