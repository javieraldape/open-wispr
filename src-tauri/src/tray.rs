...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("OpenWispr v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("OpenWispr v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
pub fn update_tray_menu(app: &AppHandle, state: &TrayIconState, locale: Option<&str>) {
    ...
    let fix_last_transcript_i = MenuItem::with_id(
        app,
        "fix_last_transcript",
        &strings.fix_last_transcript,
        true,
        None::<&str>,
    )
    .expect("failed to create fix last transcript item");
...
/// Prefer the post-processed text when present; otherwise the raw
/// transcription. Shared by the tray "Copy Last Transcript" action and the E1
/// fix command's `get_last_transcript` — one rule, not forked logic.
