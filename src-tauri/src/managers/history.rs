...
impl HistoryManager {
    ...
    pub fn get_audio_file_path(&self, file_name: &str) -> Result<PathBuf> {
        if !is_safe_recording_file_name(file_name) {
            return Err(anyhow::anyhow!("Invalid recording file name"));
        }
...
mod tests {
    ...
    #[test]
    fn recording_file_names_reject_path_traversal() {
        assert!(is_safe_recording_file_name("recording_123.wav"));
        assert!(is_safe_recording_file_name("recording_123.WAV"));
        assert!(!is_safe_recording_file_name("../secret.wav"));
        assert!(!is_safe_recording_file_name("nested/secret.wav"));
        assert!(!is_safe_recording_file_name("recording_123.mp3"));
    ...
    #[test]
    fn get_latest_completed_entry_breaks_timestamp_ties_by_newest_row() {
        let conn = setup_conn();
        insert_entry(&conn, 100, "first", None);
        insert_entry(&conn, 100, "second", None);
