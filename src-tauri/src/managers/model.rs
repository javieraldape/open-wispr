...
pub enum ModelSource {
    ...
    HuggingFace {
        repo_id: String,
        revision: String,
        /// Expected SHA-256 for the concrete file in [`ModelInfo::filename`].
        /// Curated catalog downloads must provide this; custom HF cache
        /// discoveries may leave it empty because they are already local.
        sha256: Option<String>,
    },
...
impl ModelManager {
    ...
    async fn download_hf_model(
        ...
        if repo_id.starts_with("handy-computer/") && expected_sha256.is_none() {
            return Err(anyhow::anyhow!(
                "Bundled Hugging Face model {} is missing SHA256 verification metadata",
                model_id
            ));
        }

        // Already in the shared cache (possibly from another tool)? Done.
        if let Some(cached_path) = hf_cached_path(&repo_id, &revision, &filename) {
            let verify_expected = expected_sha256.clone();
            let verify_model_id = model_id.clone();
            tokio::task::spawn_blocking(move || {
                Self::verify_sha256(&cached_path, verify_expected.as_deref(), &verify_model_id)
            })
            .await
        ...
        let downloaded_path = match repo
...
mod tests {
    ...
    fn test_discover_hf_cache_models_filters_by_arch() {
        ...
        assert!(
            matches!(&m.source, ModelSource::HuggingFace { repo_id, revision, sha256: None }
