    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        let result = {
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                match &mut engine {
                    LoadedEngine::TranscribeCpp(session) => {
                        // v1 knob-disable (see NOTICE.md "What this fork changes"):
                        // legacy custom_words no longer becomes an initial_prompt.
                        // The corrections engine (managers::corrections) is the only
                        // term-correction mechanism now, so this always passes
                        // `family: None` regardless of `settings.custom_words` —
                        // otherwise the eval would be comparing two different
                        // correction mechanisms across models. `custom_words` and
                        // `model_takes_initial_prompt` are still computed above for
                        // logging/diagnostics; only the actual prompt injection is
                        // disabled here.
                        let family: Option<RunExtension> = None;
...
fn post_process_transcription_text(
    ...
    // v1 knob-disable (see NOTICE.md "What this fork changes"): the legacy
    // fuzzy custom-words pass (`apply_custom_words`) no longer runs. The
    // corrections engine (managers::corrections) is the only term-correction
    // mechanism now — running both would make the eval compare two different
    // mechanisms across models. `apply_custom_words` itself is untouched
    // (audio_toolkit keeps its own unit tests); this call site is just no
    // longer invoked. `custom_words_already_prompted` is now unused for that
    // reason but kept as a parameter so call sites (and their `settings`
