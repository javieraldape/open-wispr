...
use crate::managers::corrections::{apply_corrections, CorrectionsManager};
...
use crate::utils::{
    self, show_linger_overlay, show_processing_overlay, show_recording_overlay,
...
fn should_hide_overlay_immediately_after_paste(paste_succeeded: bool) -> bool {
    !paste_succeeded
}
...
async fn post_process_transcription(settings: &AppSettings, transcription: &str) -> Option<String> {
    ...
    let api_key = match crate::credentials::get_post_process_api_key(&provider.id) {
        Ok(api_key) => api_key,
        Err(e) => {
            warn!(
                "Post-processing skipped because API key for provider '{}' could not be read: {}",
                provider.id, e
            );
            return None;
        }
...
pub(crate) async fn process_transcription_output(
    ...
    // emit — always last, after the optional LLM post-process above. Every
    // text-producing path in this fork converges on
    // `managers::corrections::apply_corrections` (see its doc comment); this
    // is the UI path's call site, the CLI's is in lib.rs's
    // `run_headless_transcription`.
    let corrections_manager = app.state::<Arc<CorrectionsManager>>();
    final_text = apply_corrections(&corrections_manager, &final_text);
                        ...
                        Ok(transcription) => {
                            ...
                            if processed.final_text.is_empty() {
                                ...
                                ah.run_on_main_thread(move || {
                                    ...
                                    let paste_succeeded =
                                        match utils::paste(final_text.clone(), ah_clone.clone()) {
                                            Ok(()) => {
                                                ...
                                                crate::commands::corrections::set_last_pasted_transcript(
                                                    final_text.clone(),
                                                );
                                                show_linger_overlay(&ah_clone);
                                                let _ = (PasteCompleteEvent { text: final_text })
                                                    .emit(&ah_clone);
                                                true
                                            }
                                            Err(e) => {
                                    ...
                                    if should_hide_overlay_immediately_after_paste(paste_succeeded)
                                    {
                                        utils::hide_recording_overlay(&ah_clone);
                                    }
                                    change_tray_icon(&ah_clone, TrayIconState::Idle);
...
struct FixLastTranscriptAction;

impl ShortcutAction for FixLastTranscriptAction {
    fn start(&self, app: &AppHandle, _binding_id: &str, _shortcut_str: &str) {
        if let Err(err) = crate::commands::corrections::open_fix_editor_window(app) {
            warn!("Failed to open fix editor from shortcut: {err}");
        }
    }
...
pub static ACTION_MAP: Lazy<HashMap<String, Arc<dyn ShortcutAction>>> = Lazy::new(|| {
    ...
    map.insert(
        "fix_last_transcript".to_string(),
        Arc::new(FixLastTranscriptAction) as Arc<dyn ShortcutAction>,
    );
    map.insert(
        "test".to_string(),
...
mod tests {
    ...
    #[test]
    fn successful_paste_does_not_hide_overlay_immediately() {
        assert!(!should_hide_overlay_immediately_after_paste(true));
    }
