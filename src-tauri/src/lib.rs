...
#[cfg(target_os = "macos")]
fn enforce_macos_menu_bar_app(app: &AppHandle) {
    if let Err(e) = app.set_activation_policy(tauri::ActivationPolicy::Accessory) {
        log::error!("Failed to set macOS activation policy to Accessory: {}", e);
    }
}

fn effective_tray_available(settings: &settings::AppSettings, cli_args: &CliArgs) -> bool {
    #[cfg(target_os = "macos")]
    {
        let _ = settings;
        let _ = cli_args;
...
fn show_main_window(app: &AppHandle) {
    if let Some(main_window) = app.get_webview_window("main") {
        ...
        #[cfg(target_os = "macos")]
        enforce_macos_menu_bar_app(app);
        return;
    }
...
/// Open the corrections manager, degrading to a disabled (no-correction)
/// manager if the store cannot be opened — a broken corrections DB must
/// never block dictation (see the plan's error registry).
fn new_corrections_manager(app_handle: &AppHandle) -> CorrectionsManager {
    match CorrectionsManager::new(app_handle) {
        Ok(manager) => manager,
        Err(e) => {
            log::error!("Corrections store unavailable, corrections disabled: {e}");
            CorrectionsManager::disabled()
        }
    }
}
...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    let corrections_manager = Arc::new(new_corrections_manager(app_handle));
    ...
    #[cfg(target_os = "macos")]
    enforce_macos_menu_bar_app(app_handle);
    ...
    let tray = TrayIconBuilder::new()
        ...
        .on_menu_event(|app, event| match event.id.as_ref() {
            ...
            "fix_last_transcript" => {
                if let Err(err) = commands::corrections::open_fix_editor_window(app) {
                    log::error!("Failed to open fix editor from tray: {err}");
                }
            }
            "unload_model" => {
    ...
    // Apply show_tray_icon setting. On macOS the menu bar icon is mandatory:
    // LSUIElement removes the Dock entry, so the tray icon is the only visible
...
fn run_headless_transcription(app: &AppHandle, args: &CliArgs) -> i32 {
    ...
    // T4 CHOKE POINT: corrections are the FINAL text transformation before
    // emit, on the CLI path exactly as on the UI path (see
    // `managers::corrections::apply_corrections`'s doc comment — "corrections
    // always last"). There is no LLM post-process step in the headless path,
    // so this is the only stage after raw transcription. Using the same
    // function the UI path uses (`actions.rs::process_transcription_output`)
    // is what makes the eval CLI measure the real pipeline.
    let corrections_manager = app.state::<Arc<CorrectionsManager>>();
    text = managers::corrections::apply_corrections(&corrections_manager, &text);
...
pub fn run(cli_args: CliArgs) {
    ...
    let specta_builder = Builder::<tauri::Wry>::new()
        .commands(collect_commands![
            ...
            commands::request_input_monitoring_access,
            ...
            commands::corrections::get_last_transcript,
            commands::corrections::save_transcript_fix,
            commands::corrections::preview_fix,
            commands::corrections::corrections_list,
            commands::corrections::corrections_delete,
            commands::corrections::corrections_add_manual,
            commands::corrections::open_fix_editor,
            helpers::clamshell::is_laptop,
        ])
        .events(collect_events![
            actions::PasteCompleteEvent,
    ...
    builder
        ...
        .setup(move |app| {
            ...
            if headless_mode {
                ...
                // The corrections manager must be reachable here too — the
                // eval CLI measures the real pipeline, and "corrections
                // always last" (T4) requires the headless path to apply the
                // same choke point as the UI path (see
                // `run_headless_transcription`, which calls
                // `managers::corrections::apply_corrections`).
                let corrections_manager = Arc::new(new_corrections_manager(&app_handle));
                app_handle.manage(corrections_manager);
                managers::transcription::init_transcribe_backend();
                managers::transcription::apply_accelerator_settings(&app_handle);
            ...
            // Hide tray icon if --no-tray was passed. Ignored on macOS because
            // the app is Dockless there and must keep a menu bar entry point.
            #[cfg(not(target_os = "macos"))]
            if cli_args.no_tray {
                tray::set_tray_visibility(&app_handle, false);
            }
            ...
            let tray_available = effective_tray_available(&settings, &cli_args);
        ...
        .on_window_event(|window, event| match event {
            tauri::WindowEvent::CloseRequested { api, .. } => {
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    let tray_visible = effective_tray_available(&settings, &cli_args);
                    if tray_visible {
                        // Tray is available: hide the dock icon, app lives in the tray
                        enforce_macos_menu_bar_app(window.app_handle());
                    }
