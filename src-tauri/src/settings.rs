...
pub fn get_default_settings() -> AppSettings {
    ...
    bindings.insert(
        ...
        ShortcutBinding {
            id: "fix_last_transcript".to_string(),
            name: "Fix Last Transcript".to_string(),
            description: "Opens the editor for teaching a correction from the last transcript."
                .to_string(),
            default_binding: "option+cmd+f".to_string(),
            current_binding: "option+cmd+f".to_string(),
        },
    );
...
pub fn load_or_create_app_settings(app: &AppHandle) -> AppSettings {
    ...
    let keys_changed = crate::credentials::sanitize_post_process_api_key_settings(&mut settings);
...
pub fn get_settings(app: &AppHandle) -> AppSettings {
    ...
    let mut settings = if let Some(settings_value) = store.get("settings") {
        match serde_json::from_value::<AppSettings>(settings_value.clone()) {
            Ok(mut settings) => {
                let migrations_changed = apply_settings_migrations(&mut settings, &settings_value);
                let keys_changed =
                    crate::credentials::sanitize_post_process_api_key_settings(&mut settings);
                if migrations_changed || keys_changed {
                    store.set("settings", serde_json::to_value(&settings).unwrap());
                }
                settings
            }
