...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::HandyKeys => handy_keys::validate_app_shortcut(raw),
    }
}
...
pub fn change_post_process_api_key_setting(
    ...
    crate::credentials::set_post_process_api_key(&provider_id, &api_key)?;
    settings.post_process_api_keys.insert(
        provider_id,
        crate::credentials::marker_for_api_key(&api_key),
    );
    settings::write_settings(&app, settings);
    Ok(())
}
...
pub async fn fetch_post_process_models(
    ...
    let api_key = crate::credentials::get_post_process_api_key(&provider_id)?;
...
#[tauri::command]
#[specta::specta]
pub fn change_show_tray_icon_setting(app: AppHandle, _enabled: bool) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        let mut settings = settings::get_settings(&app);
    ...
    {
        let mut settings = settings::get_settings(&app);
        settings.show_tray_icon = _enabled;
        settings::write_settings(&app, settings);

        // Apply change immediately
        tray::set_tray_visibility(&app, _enabled);
