...
const AccessibilityPermissions: React.FC = () => {
  ...
  const [hasKeyboardPermissions, setHasKeyboardPermissions] =
  ...
  const handleButtonClick = async (): Promise<void> => {
    ...
    try {
      const [hasAccessibility, hasInputMonitoring] = await Promise.all([
        checkAccessibilityPermission(),
        checkInputMonitoringPermission(),
      ]);
      ...
      if (!hasAccessibility) {
        ...
        setHasKeyboardPermissions(true);
  ...
  useEffect(() => {
    ...
    const initialSetup = async (): Promise<void> => {
      ...
      const hasPermissions = hasAccessibility && hasInputMonitoring;
      setHasKeyboardPermissions(hasPermissions);
  ...
  if (!isMacOS || hasKeyboardPermissions) {
    return null;
  }
