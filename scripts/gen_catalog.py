...
def gguf_files(repo, siblings):
    """Return GGUF files with mandatory size and checksum metadata.
...
def build(repo):
    ...
    if not isinstance(info.sha, str) or not re.fullmatch(r"[0-9a-fA-F]{40}", info.sha):
        raise ValueError(f"{repo}: missing pinned commit revision")
